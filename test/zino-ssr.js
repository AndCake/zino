...
test('can load a simple component', t => {
	...
	t.is(result, '<server-test-simple __ready="true"><div class="-shadow-root">test</div></server-test-simple>');
});

test('can deal with styles', t => {
	let result = renderComponent('server-test-styles', './test/components/server-test-styles');
	t.is(result, '<style id="style:server-test-styles">server-test-styles{background: red;}</style><server-test-styles __ready="true"><div class="-shadow-root">test</div></server-test-styles>');
});
