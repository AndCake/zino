	...
	for (var all in functions) {
		...
		if (['mount', 'unmount', 'events', 'render'].indexOf(all) < 0) {
			if (isFn(entry)) {
				...
				tag[all] = isObj(tag[all]) ? merge$1({}, entry, tag[all]) : entry;
			}
		}
	}
...
function renderTag(tag) {
	...
	do {
		...
		renderedSubElements.length > 0 && (tag.querySelectorAll && toArray$1(tag.querySelectorAll('[__ready]')) || []).forEach(function (subEl, index, arr) {
